...
pytest tests/unit/test_{feature}.py -v — all new tests pass
pytest tests/ -m "not slow" --tb=short — no regressions
pytest tests/ --cov=src --cov-report=term-missing — coverage increased
New test file registered in correct directory with __init__.py
New pytest markers registered in pyproject.toml if used